...
def build_dump_cmds(db_host, db_port, db_user, db_pass, auth_db,
                    db_collections, dump_path, use_oplog=False):
    cmds = []
...
def build_restore_cmd(db_host, db_port, db_user, db_pass, auth_db,
                      db_collections, restore_path, use_oplog_replay=False):
    cmd = [
        "mongorestore",
        f"--host={db_host}",
        f"--port={db_port}",
...
def main():
    ...
    parser.add_argument("--oplog", action="store_true",
                        help="Use --oplog for consistent dump (requires replica set)")
    parser.add_argument("--oplogReplay", action="store_true",
                        help="Use --oplogReplay during restore")
    ...
    if args.restore:
        ...
        restore_cmd = build_restore_cmd(
            ...
            db_collections,
            restore_path,
            use_oplog_replay=args.oplogReplay
        )
